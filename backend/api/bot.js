...
bot.on('text', async (ctx) => {
  ...
  if (session.step === 'waiting_password') {
    ...
    try {
      ...
      console.log('Message delete failed, maybe bot has no admin rights', e);
    }
  ...
  if (session.step === 'authenticated') {
    ...
    // To'g'ridan to'g'ri link beramiz, bot orqali jo'natish xotirada muammo tug'diradi
...
bot.on(['document', 'photo', 'video', 'audio'], async (ctx) => {
  ...
  try {
    ...
    // Telegram'dan fayl ma'lumotlarini olish
    ...
    // Telegram link o'rniga fileURL olish
